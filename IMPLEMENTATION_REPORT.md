...
- Pricing and availability is **Complete** in the Partner Center draft: free (INR 0), available in all worldwide markets including future markets, public and discoverable, with release set for as soon as certification allows. This is draft configuration only; the app is not yet public.
- At the initial draft stage, Properties had category **Personal finance / Banking + investments** and website `https://harsh-91.github.io/neon-ledger/`, but still needed a privacy policy. The later preparation below records the corrected support contact and completed policy.

### Store listing and privacy preparation (26 September 2026)

- Corrected the Partner Center support contact to `harshnair02@hotmail.com` and verified that the saved value persisted. Corrected the app's diagnostic-email draft, diagnostics page, README and a unit test locally; prepared version 1.6.1. The already uploaded 1.6.0 MSIX still contains the old address and must be replaced by a validated 1.6.1 package before certification.
- Audited local data flows and drafted a privacy policy covering statement processing, temporary mapping files, optional update checks, opt-in API/MCP access, support reports, Windows backup and third-party services. Saved the policy text in Partner Center; **Properties is Complete**. A matching `privacy.html` and footer link are ready in the separate Neon Ledger website repository but not yet published because the current GitHub browser identity lacks push access to `harsh-91/neon-ledger`.
- Completed the IARC utility questionnaire and obtained an all-ages preview. The age rating is **In Progress** pending the creator's own IARC Terms-of-Use and age-of-majority attestation; no agreement was accepted on the creator's behalf.
- Saved the English Store description, short description, feature, developer and copyright text. Store listings remain **Incomplete** because a genuine, privacy-safe desktop screenshot is still required. A synthetic zero-data preview was inspected but not uploaded.
- Saved a `runFullTrust` explanation and changed Submission Options to **Don't publish until I select Publish now**. The explanation persisted, but Partner Center still marks Submission Options **Incomplete**; investigate after other required sections are complete. Certification test notes were saved and verified after reload. The draft has not been submitted or published.
- All 36 unit tests passed (one opt-in compiled-installer integration test skipped). No production package was rebuilt or uploaded during this step.
