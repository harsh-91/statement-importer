...
Download `StatementImporter-1.3.4-Setup-x64.exe` from the repository's [latest release](https://github.com/harsh-91/statement-importer/releases/latest). The standard Windows wizard provides per-user installation, direct shutdown of the prior app during upgrades, shortcut choices, Programs & Features registration, and a data-preserving uninstaller.
...
When Inno Setup 6/7 or the project-local compiler is available, the build also produces the conventional `StatementImporter-1.3.4-Setup-x64.exe` installer from `installer\StatementImporter.iss`.
